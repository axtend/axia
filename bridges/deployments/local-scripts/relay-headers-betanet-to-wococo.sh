...
RUST_LOG=rpc=trace,bridge=trace ./target/debug/substrate-relay init-bridge betanet-to-wococo \
...
RUST_LOG=rpc=trace,bridge=trace ./target/debug/substrate-relay relay-headers betanet-to-wococo \
	--source-host 127.0.0.1 \
	--source-port 9955 \
	--target-host 127.0.0.1 \
	--target-port 9944 \

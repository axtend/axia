...
RUST_LOG=rpc=trace,bridge=trace ./target/debug/parity-relay init-bridge rococo-to-wococo \
...
RUST_LOG=rpc=trace,bridge=trace ./target/debug/parity-relay relay-headers rococo-to-wococo \
	--source-host 127.0.0.1 \
	--source-port 9955 \
	--target-host 127.0.0.1 \
	--target-port 9944 \

...
In the scenarios, for simplicity, we call the chains Kusama (KSM token) and Axia (DOT token),
...
Kusama's Alice (kAlice) receives 5 DOTs from Axia's Bob (pBob) and sends half of them to
kCharlie.
...
Scenario 7: Axia's Bob spending his DOTs by using Kusama chain
---------------------------
